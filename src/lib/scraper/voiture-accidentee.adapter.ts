...
const VA_SELECTORS: ScraperSelectors = {
  listingContainer: '[class*="item"], [class*="card"], [class*="vehicle"], article',
  ...
  imageUrl: 'img',
  link: 'a[href*="fiche"], a[href*="vehicule"], a[href*="occasion"]',
  nextPage: '[rel="next"], a[class*="next"]',
};

export class VoitureAccidenteeAdapter extends BaseAdapter {
  name = "voiture-accidentee";

  async scrape(url: string, selectors?: ScraperSelectors): Promise<RawListing[]> {
    const applied = { ...VA_SELECTORS, ...selectors };
