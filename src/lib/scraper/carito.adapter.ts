...
const CARITO_SELECTORS: ScraperSelectors = {
  listingContainer: '[class*="listing-item"], [class*="card"], [class*="vehicle"], article',
...
export class CaritoAdapter extends BaseAdapter {
  name = "carito";

  async scrape(url: string, selectors?: ScraperSelectors): Promise<RawListing[]> {
    const applied = { ...CARITO_SELECTORS, ...selectors };
