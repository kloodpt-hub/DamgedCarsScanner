...
export function JobsClientBar({
  ...
  useEffect(() => {
    if (!autoRefresh || !hasRunning) {
      if (autoRefresh && !hasRunning) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- turning the auto-refresh toggle back off when no job is running is a prop-change state reset; deriving it instead would leave the toggle visibly on
